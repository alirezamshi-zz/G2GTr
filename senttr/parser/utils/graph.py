## the base code is from https://github.com/huggingface/transformers
from transformers.modeling_bert import load_tf_weights_in_bert,BERT_PRETRAINED_MODEL_ARCHIVE_MAP,gelu,gelu_new,\
     swish,mish,ACT2FN
...
from transformers.modeling_bert import BertSelfOutput,BertIntermediate,BertOutput,BertPooler,\
...
class BertGraphEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.label_embeddings = nn.Embedding(config.label_size, config.hidden_size,padding_idx=0)
    ...
    def forward(self, input_ids=None, pos_ids=None, graph_rel=None, token_type_ids=None,
        ...
        if position_ids is None:
            ...
            if seq_length >= self.max_position:
                position_id_c = torch.arange(-self.max_position,seq_length-self.max_position,
                                             dtype=torch.long,device=device)
                position_ids =position_ids * (position_ids < self.max_position).long() \
                              + position_id_c * (position_ids >= self.max_position).long()
        ...
        if graph_rel is not None:
            ...
            embeddings = inputs_embeds + position_embeddings + token_type_embeddings + label_embeddings
        else:
...
class BertGraphSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        if self.layernorm_value:
            self.layernorm_value_layer = nn.LayerNorm(self.attention_head_size)

        ## embedding for dependency graph
        self.num_dp = 3
...
class BertGraphAttention(nn.Module):
    ...
    def forward(
        ...
        self_outputs = self.self(
            hidden_states, attention_mask, head_mask, encoder_hidden_states, encoder_attention_mask, graph_arc
        )
...
class BertGraphLayer(nn.Module):
    ...
    def forward(
        ...
        self_attention_outputs = self.attention(hidden_states, attention_mask, head_mask, graph_arc)
    ...
    def forward(
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            layer_outputs = layer_module(
                hidden_states, attention_mask, head_mask[i], encoder_hidden_states, encoder_attention_mask, graph_arc
            )
...
class BertGraphModel(BertPreTrainedModel):
    ...
    def forward(
        ...
        embedding_output = self.embeddings(
            input_ids=input_ids, pos_ids=pos_ids, graph_rel=graph_rel, 
